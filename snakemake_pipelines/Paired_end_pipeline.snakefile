...
STUDY = config["study2"]["name"]
LOG_DIR = "logs/" + STUDY + "/"
WORK_DIR = "data/" + STUDY + "/"
RESULT_DIR = "results/" + STUDY + "/"
BENCHMARK_DIR="benchmarks/" + STUDY + "/"

READ_LEN = config["study2"]["length"]
df = pd.read_csv(config["study2"]["route"], sep="\t")
...
rule all:
	input:
#		expand(RESULT_DIR + "fastq/{sample}.sra_{rg}.fastq", rg=["1","2"], sample=samples),
#		expand(RESULT_DIR + "fastqc/{sample}.sra_{rg}_fastqc.{ext}", rg=["1","2"], ext=["html","zip"], sample=samples),
		RESULT_DIR + "multiqc/multiqc.html",
		RESULT_DIR + "multiqc/trim_multiqc.html",
#		expand(RESULT_DIR + "trimmed/{sample}.sra_{rg}_val_{rg}.fq.gz", rg = ["1","2"], sample=samples),
#		expand(RESULT_DIR + "trimmed/{sample}.sra_{rg}.fastq_trimming_report.txt", rg=["1","2"], sample=samples),
...
rule Fasterq_dump:
	...
	output:
		temp(RESULT_DIR + "fastq/{sample}.sra_1.fastq"),
		temp(RESULT_DIR + "fastq/{sample}.sra_2.fastq")
	params:
	...
	shell:
		"""
		(fasterq-dump --split-3 -e {threads} {input} --outdir {params.outdir}) &> {log}
		"""

rule TrimGalore:
	input:
		RESULT_DIR + "fastq/{sample}.sra_1.fastq",
		RESULT_DIR + "fastq/{sample}.sra_2.fastq"
	output:
		temp(RESULT_DIR + "trimmed/{sample}.sra_1_val_1.fq.gz"),
		temp(RESULT_DIR + "trimmed/{sample}.sra_2_val_2.fq.gz"),
		RESULT_DIR + "trimmed/{sample}.sra_1.fastq_trimming_report.txt",
		RESULT_DIR + "trimmed/{sample}.sra_2.fastq_trimming_report.txt"
	params:
		outdir = RESULT_DIR + "trimmed/"
	benchmark:
		BENCHMARK_DIR+"trim_galore/{sample}.trimmed.benchmark.txt"
	threads:2
	conda:
	...
	priority: 11
	shell:
		"(trim_galore --phred33 --gzip --output_dir {params.outdir} --paired {input[0]} {input[1]}) 2> {log}"


rule FastQC:
	input:
		fq1 = RESULT_DIR + "fastq/{sample}.sra_1.fastq",
		fq2 = RESULT_DIR + "fastq/{sample}.sra_2.fastq",
		trim_fq1 = RESULT_DIR + "trimmed/{sample}.sra_1_val_1.fq.gz",
		trim_fq2 = RESULT_DIR + "trimmed/{sample}.sra_2_val_2.fq.gz"
	output:
		RESULT_DIR + "fastqc/{sample}.sra_1_fastqc.html",
		RESULT_DIR + "fastqc/{sample}.sra_1_fastqc.zip",
		RESULT_DIR + "fastqc/{sample}.sra_2_fastqc.html",
		RESULT_DIR + "fastqc/{sample}.sra_2_fastqc.zip",
		RESULT_DIR + "trimmed_qc/{sample}.sra_1_val_1_fastqc.zip",
		RESULT_DIR + "trimmed_qc/{sample}.sra_1_val_1_fastqc.html",
		RESULT_DIR + "trimmed_qc/{sample}.sra_2_val_2_fastqc.zip",
		RESULT_DIR + "trimmed_qc/{sample}.sra_2_val_2_fastqc.html"
	benchmark:
	...
	shell:
		"""
		fastqc -o {params.outdir1} --noextract {input.fq1} {input.fq2} &> {log.log1}
		fastqc -o {params.outdir2} --noextract {input.trim_fq1} {input.trim_fq2} &> {log.log2}
		"""

rule MultiQC:
	input:
		expand(RESULT_DIR + "fastqc/{sample}.sra_{rg}_fastqc.{ext}", rg =["1","2"], ext=["html","zip"], sample=samples),
		expand(RESULT_DIR + "trimmed_qc/{sample}.sra_{rg}_val_{rg}_{ext}",rg=["1","2"],ext=["fastqc.html","fastqc.zip"],sample=samples)
	output:
		RESULT_DIR + "multiqc/multiqc.html",
		RESULT_DIR + "multiqc/trim_multiqc.html"
	params:
...
rule STAR_Align:
	input:
		RESULT_DIR + "trimmed/{sample}.sra_1_val_1.fq.gz",
		RESULT_DIR + "trimmed/{sample}.sra_2_val_2.fq.gz"
	output:
		...
		RESULT_DIR + "star_align/{sample}_Unmapped.out.mate2"
	params:
	...
	shell:
		...
		--genomeDir {params.index}  --readFilesIn {input[0]} {input[1]} --outFileNamePrefix {params.sample}) &> {log}
		"""
...
rule Reads_distribution:
	...
	conda:
		"envs/RseQC.yaml"
	log:
		LOG_DIR + "read_distribution/{sample}_read_distribution.log"
	message:
...
rule BAM_filter:
	...
	priority: 13
	shell:
		"(samtools view -f 3 -F 3596 {input} -o {output}) &> {log}"
...
rule Exon_number:
	...
	benchmark:
		BENCHMARK_DIR + "htseq_count_exon/{sample}.exon_count.benchmark.txt"
	log:
		LOG_DIR + "htseq_count_exon/{sample}.exon_count.log"
	conda:
		"envs/htseq_count.yaml"
	message:
		"Executing exon count on the following files {input}."
	priority: 14
	shell:
...
rule Kraken2_classify:
	input:
		...
		fq2 = RESULT_DIR + "star_align/{sample}_Unmapped.out.mate2"
	output:
	...
	conda:
		"envs/kraken2.yaml"
	log:
		LOG_DIR + "kraken2_classify/{sample}_{type}_kraken2.log"
	message:
		"Executing Kraken2 on the following files {input}."
	priority: 3
	shell:
		"""
		kraken2 --db {params.db} --threads {threads} {params.extra} --report {output.krp} --output {output.kop} {input.fq1} {input.fq2} &> {log}
		"""
